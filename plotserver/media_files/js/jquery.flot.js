    ...
    function Plot(placeholder, data_, options_, plugins) {
        ...
        var series = [],
            options = {
                ...
                grid: {
                    show: true,
                    above: false, // True if the grid lines are drawn above the data
                    color: "#545454", // primary color used for outline and labels
                    backgroundColor: null, // null for transparent, else color
                    tickColor: "rgba(0,0,0,0.2)", // color used for the ticks
        ...
        $(window).resize(function() {
            ...
            // remember selection
            var px1 = selection.first.x/plotWidth,
                py1 = selection.first.y/plotHeight,
                px2 = selection.second.x/plotWidth,
                py2 = selection.second.y/plotHeight;
            
            // resize canvas
            canvasWidth = canvas.width = overlay.width = w;
            canvasHeight = canvas.height = overlay.height = h;
            plot.setupGrid();
            plot.draw();

            // reset selection
            selection.first.x = px1*plotWidth;
            selection.first.y = py1*plotHeight;
            selection.second.x = px2*plotWidth;
            selection.second.y = py2*plotHeight;
            ...
            function measureLabels(axis, axisOptions) {
                ...
                if (axis == axes.xaxis || axis == axes.x2axis) {
                    ...
                    if (axis.labelWidth == null) {
                        var dummyDiv = $('<div style="position:absolute;top:-10000px;font-size:smaller">88888888</div>').appendTo(placeholder);
        ...
        function draw() {
            ...
            if (options.grid.show && !options.grid.above)
                drawGrid();
